...
public class Shield : MonoBehaviour
{
    ...
    public void OnEnable() //This method is called before any other method (except Awake).
    {

        GettingShieldData(); //Calls the GettingShieldData method.

    }

    public void GettingShieldData() //This method gets the shield the player currently has equipped in their inventory, saves it to this class and then calls another method that will apply this data.
    {

        shieldData = (ShieldData)EquipmentManager.instance.equippedEquipment[3];

        ApplyingShieldData(); //Calls the ApplyingShieldData method.

    }

    void ApplyingShieldData() //This method accesses the public variables stored within the saved shieldData ShieldData script and saves them in this class's global variables.
    {
        ...
        level = shieldData.level;
        baseCapacity = shieldData.baseCapacity;
        baseRechargeRate = shieldData.baseRechargeRate;
        rechargeDelay = shieldData.rechargeDelay;

        baseSellValue = shieldData.baseSellValue;

        CalculatingVariables(); //Calls the CalculatingVariables method.

    }

    void CalculatingVariables() //This method determines the capacity, rechargeRate, sell and buy value of the shield depending on the shield's level.
    {
    ...
    public void ResettingShield() //This method sets the currentCapacity of the shield to 0 so it has to be regenerated again.
    {

        currentCapacity = 0; //Sets the currentCapacity to 0.
        StopAllCoroutines(); //Calls the StopAllCoroutines method.
    ...
    void Update()
    {
        ...
        TakeDamage(); //Calls the TookDamage method.

        CheckShieldParameters(); //Calls the CheckShieldParameters method.
